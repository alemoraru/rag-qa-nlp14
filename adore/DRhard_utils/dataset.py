...
class TrainInbatchDataset(Dataset):
    def __init__(
        self, rel_file, queryids_cache, docids_cache, max_query_length, max_doc_length
    ):
...
def dual_get_collate_function(
    max_query_length, max_doc_length, rel_dict, padding=False
):
...
def triple_get_collate_function(
    max_query_length, max_doc_length, rel_dict, padding=False
):
    query_collate_func = single_get_collate_function(max_query_length, padding)
    doc_collate_func = single_get_collate_function(max_doc_length, padding)
    ...
    def collate_function(batch):
        ...
        input_data = {
            ...
            "input_doc_ids": doc_data["input_ids"],
            "doc_attention_mask": doc_data["attention_mask"],
            "other_doc_ids": hard_doc_data["input_ids"].reshape(
                query_num, hard_num_per_query, -1
            ),
            "other_doc_attention_mask": hard_doc_data["attention_mask"].reshape(
                query_num, hard_num_per_query, -1
            ),
            "rel_pair_mask": torch.FloatTensor(rel_pair_mask),
