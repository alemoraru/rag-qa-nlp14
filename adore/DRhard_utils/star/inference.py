...
def prediction(
    model, data_collator, args, test_dataset, embedding_memmap, ids_memmap, is_query
):
...
def doc_inference(model, args, embedding_size):
    ...
    doc_dataset = SubsetSeqDataset(
        subset=subset, ids_cache=ids_cache, max_seq_length=args.max_doc_length
    )
    assert not os.path.exists(args.doc_memmap_path)
    doc_memmap = np.memmap(
        args.doc_memmap_path,
        dtype=np.float32,
        mode="r",
        shape=(len(doc_dataset), embedding_size),
    )
    docid_memmap = np.memmap(
        args.docid_memmap_path, dtype=np.int32, mode="r", shape=(len(doc_dataset),)
    )
    try:
        prediction(
            model,
...
def main():
    ...
    parser.add_argument(
        "--data_type", choices=["passage", "doc"], type=str, required=True
    )
    ...
    args.queryids_memmap_path = os.path.join(
        args.output_dir, f"{args.mode}-query-id.memmap"
    )
    ...
    config = RobertaConfig.from_pretrained(
        args.model_path, gradient_checkpointing=False
    )
