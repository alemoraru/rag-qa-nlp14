...
class RobertaDot(BaseModelDot, RobertaPreTrainedModel):
    def __init__(self, config, model_argobj=None):
        ...
        )  # AutoModel.from_pretrained("../rag-qa-nlp14/star_model", device_map='cpu')
...
def inbatch_train(
    ...
    with autocast(enabled=False):
        ...
        positive_scores = (
            single_positive_scores.reshape(-1, 1).repeat(1, batch_size).reshape(-1)
        )
        if rel_pair_mask is None:
            rel_pair_mask = 1 - torch.eye(
                batch_size, dtype=batch_scores.dtype, device=batch_scores.device
            )
        # print("mask", mask)
        batch_scores = batch_scores.reshape(-1)
        logit_matrix = torch.cat(
            [positive_scores.unsqueeze(1), batch_scores.unsqueeze(1)], dim=1
        )
        # print(logit_matrix)
    ...
    with autocast(enabled=False):
        ...
        positive_scores = (
            single_positive_scores.reshape(-1, 1).repeat(1, other_doc_num).reshape(-1)
        )
        other_logit_matrix = torch.cat(
            [positive_scores.unsqueeze(1), other_batch_scores.unsqueeze(1)], dim=1
        )
        # print(logit_matrix)
