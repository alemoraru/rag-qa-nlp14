...
def train(args, model):
    ...
    tb_writer = SummaryWriter(
        os.path.join(args.log_dir, time.strftime("%b-%d_%H:%M:%S", time.localtime()))
    )
    passage_embeddings = np.memmap(
        args.pembed_path, dtype=np.float32, mode="r"
    ...
    for epoch_idx, _ in enumerate(train_iterator):
        ...
        for step, (batch, all_rel_pids) in enumerate(epoch_iterator):
            ...
            I_nearest_neighbor = index.search(
                query_embeddings.detach().cpu().numpy(), args.neg_topk
            )[1]
            ...
            for retrieve_pids, cur_rel_pids, qembedding in zip(
                ...
                if np.sum(target_labels) == 0:
                    ...
                    target_labels = np.hstack(
                        [target_labels, [True] * len(cur_rel_pids)]
                    )
                ...
                topK_passage_embeddings = torch.FloatTensor(
                    passage_embeddings[retrieve_pids]
                ).to(args.model_device)
                y_pred = (qembedding.unsqueeze(0) * topK_passage_embeddings).sum(
                    -1, keepdim=True
                )
                sigma = 1
...
def main():
    ...
    args.model_device = torch.device(
        "cpu"
    )  # torch.device(f"cuda:{args.model_gpu_index}")
