...
class EvalPipeline:
    def __init__(self, model = "llama", golden_evaluation = False):
        ...
        self.llm_instance = LLMEngineOrchestrator.get_llm_engine(llm_class="llama", data = "", model_name="meta-llama/Llama-3.2-1B-Instruct") # add model_name="meta-llama/Llama-2-7b-chat-hf", temperature=0.3, top_n=1, top_p=None
    ...
    def evaluate_queries(self, queries: list[Query]):
        ...
        """
        prompt_type = "docoument"
        if(self.golden_eval):
            prompt_type = "context"

        for query in queries:
            prompt, documents = self.create_promt(query, prompt_type)
    ...
    def create_promt(self, query:Query, prompt_type = "docoument"): 
        """
        Create a promt to the query contains either the context (gold docs) of the query (prompt_type=context) 
...
def perform_evaluation(sampling, k = 1):
    ...
    print(f"Sampling on {sampling} with k {k} with result: correct answers {correct_answers} out of total {len(answers)} -> {correct_answers/len(answers)}")
